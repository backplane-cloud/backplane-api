  ...
  if (jsonObject.length !== 0) {
    ...
    jsonObject.map((entity) => {
      ...
      fields.map((field, i) => {
        ...
        let meta =
          i === 0
            ? ` class="cursor-pointer px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white" hx-get="/api/${type}/${entity.id}" hx-target="#datapane" hx-headers='{"ui": true}'`
            : `class="px-6 py-4"`;
